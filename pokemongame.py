...
class Pokemon(pygame.sprite.Sprite):
    ...
    def __init__(self, name, level, x, y):
        ...
        self.types=[self.json['types'][i]['type']['name']for i in range(len(self.json['types']))]
        self.size = 150
    ...
    def perform_attack(self, other, move):
        ...
        if move.type in self.types:damage*=1.5
        if random.randint(1, 10000)<=625:damage*=1.5
    ...
    def set_sprite(self, side):
        self.image = pygame.image.load(io.BytesIO(urlopen(self.json['sprites'][side]).read())).convert_alpha()
    ...
    def draw_hp(self):
        ...
        for i in range(self.max_hp):pygame.draw.rect(game,red,(self.hp_x+bar_scale*i,self.hp_y,bar_scale,20))
        for i in range(self.current_hp):pygame.draw.rect(game,green,(self.hp_x+bar_scale*i,self.hp_y,bar_scale,20))

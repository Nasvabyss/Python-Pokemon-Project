...
class Frame(Node):
    def __init__(self,node,figma_file,output_path,frameCount=0):
        ...
        self.elements=[self.create_element(child)for child in self.children if Node(child).visible]
    def create_element(self,element):
        ...
        if element_name=='button':
            ...
            image_path=(self.assets_path/f'button_{self.counter[Button]}.png')
            download_image(image_url,image_path)
            image_path=image_path.relative_to(self.assets_path)
            return Button(element,self,image_path,id_=f'{self.counter[Button]}')
            ...
            download_image(image_url,image_path)
            image_path=image_path.relative_to(self.assets_path)
            return TextEntry(element,self,image_path,id_=f'{self.counter[TextEntry]}')
        ...
        if element_name=='line' or element_type=='line':return Line(element,self)
        elif element_type=='text':return Text(element,self)
        else:
    ...
    def to_code(self,template):return Template(template).render(window=self,elements=self.elements,assets_path=self.assets_path)
class Group(Frame):
...
class Instance(Frame):
    def __init__(self,node):super().__init__(node)
    @property
    def component_id(self)->str:self.node.get('componentId')

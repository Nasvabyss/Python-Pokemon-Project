...
class Vector(Node):
    ...
    def position(self,frame):
        ...
        return abs(x-frame_x),abs(y-frame_y)
class Star(Vector):
    def __init__(self,node):super().__init__(node)
...
class Rectangle(Vector):
    ...
    @property
    def corner_radius(self):return self.node.get('cornerRadius')
    @property
    def rectangle_corner_radii(self):return self.node.get('rectangleCornerRadii')
    def to_code(self):return f'''canvas.create_rectangle({self.x},{self.y},{self.x+self.width},{self.y+self.height},fill='{self.fill_color}',outline='')'''
class Line(Rectangle):
    def __init__(self,node,frame):super().__init__(node,frame)
    def color(self)->str:
        '''Returns HEX form of element RGB color (str)'''
        try:
...
class UnknownElement(Vector):
    def __init__(self,node,frame):
        ...
        self.x,self.y=self.position(frame)
        self.width,self.height=self.size()
    def to_code(self):return f'''canvas.create_rectangle({self.x},{self.y},{self.x+self.width},{self.y+self.height},fill='#000000',outline='')'''

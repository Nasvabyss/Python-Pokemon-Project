...
from logging import debug, warn, critical  # Logging
...
def imgCreator(pokemon, data=False):
    ...
    try:
        ...
        critical('The website used to obtain essential images has been blocked by your network. If this error reoccurs, please try another network.')
        exit()
    DATA = URL.read()
    URL.close()
    endTime = time()
    debug(f'Successfully retrieved photo for {pokemon} ({endTime-startTime}s)')
